...
**Author:** Sutikshn Koushik  
**GitHub:** [SutikshnKoushik](https://github.com/SutikshnKoushik)  
**Modified from:** [ExpenseOwl](https://github.com/Tanq16/ExpenseOwl) by Tanq16

---

smart-budget is a self-hosted expense tracking application with Google OAuth and email authentication, budget limits, and beautiful visualizations.

<p align="center">
<img src="/assets/logo.png" alt="smart-budget Logo" width="200" height="200" /><br>
</p>

<h1 align="center">smart-budget</h1><br>

<p align="center">
<a href="https://github.com/SutikshnKoushik/smart-budget/actions/workflows/release.yml"><img src="https://github.com/SutikshnKoushik/smart-budget/actions/workflows/release.yml/badge.svg" alt="Release"></a>&nbsp;<a href="https://github.com/SutikshnKoushik/smart-budget/releases"><img alt="GitHub Release" src="https://img.shields.io/github/v/release/SutikshnKoushik/smart-budget"></a>&nbsp;<a href="https://hub.docker.com/r/SutikshnKoushik/smart-budget"><img alt="Docker Pulls" src="https://img.shields.io/docker/pulls/SutikshnKoushik/smart-budget"></a>
</p>
...
<p align="center">
<b>smart-budget</b> is an extremely simple self-hosted expense tracking system with a modern monthly pie-chart visualization and cashflow showcase.
</p>
...
The front end of smart-budget can be installed as a Progressive Web App on desktop and mobile devices (i.e., the back end still needs to be self-hosted). To install:
...
docker run --rm -d \
  --name smart-budget \
  -p 8080:8080 \
  -v smart-budget:/app/data \
  SutikshnKoushik/smart-budget:main
...
services:
  smart-budget:
    image: SutikshnKoushik/smart-budget:main
    restart: unless-stopped
    ports:
      - 5006:8080 # change 5006 to what you want to expose on
    volumes:
      - /home/tanq/smart-budget:/app/data # change dir as needed
...
```bash
git clone https://github.com/SutikshnKoushik/smart-budget.git && \
cd smart-budget && \
go build ./cmd/smart-budget
```
...
> [!NOTE]
> This app does not include authentication, so deploy carefully. I don't want to add half-baked authentication, so use Authelia, or equivalent as needed. smart-budget works well with a reverse proxy like Nginx Proxy Manager too and is intended for homelab use only.
...
- The primary way to use smart-budget is to quick review the month's stats via the pie chart - this allows users to make a mental note and soft decision of where to spend money, without the effort of maintaining a budget
...
> [!NOTE]
> While these conventions can change during the project's lifecycle, largely, the intention (stemming from the motivation to build smart-budget) behind simple, manual, easy tracking will not change.

### Configuration Options

With the exception of [Data backends](#data-backends), all configuration of smart-budget happens via the application UI. The list of all such options available via the settings page (`/settings` endpoint) is as follows:
...
smart-budget supports two data backends - JSON (default), and Postgres. Postgres was added with v4.0 of the app primarily for homelabbers to reuse their Postgres instances as needed for better backup compatibility.

Ideally, you need not configure anything differently for the JSON backend. smart-budget automatically creates the data directory and the `.json` files. You may, however, want to mount a specific volume to `/app/data` within the container for persistence.
...
| STORAGE_URL | "localhost:5432/smart-budgetdb" | format - SERVER/DB - the sslmode value is set by the next variable |
...
smart-budget is meant to make things simple, and importing CSV abides by the same philosophy. smart-budget will accept any CSV file as long as it contains the columns - `name`, `category`, `amount`, and `date`. This is case-insensitive so `name` or `Name` doesn't matter.

> [!TIP]
> This feature allows smart-budget to use exported data from any tool as long as the required categories are present, making it insanely easy to shift from any provider.

> [!WARNING]
> The recommended format for the date is RFC3339. Additionally, smart-budget can ingest several other time formats, including a short, human written date like `2012/8/14` (14th August 2012).
> HOWEVER !!!
> smart-budget only ingests date in YYYY-MM-DD (this order). smart-budget does NOT deal with MM/DD or DD/MM. Full 4 digit year comes first, followed by month, and lastly the date.

> [!NOTE]
> smart-budget goes through every row in the imported data, and will intelligently fail on rows that have invalid or absent data. There is a 10 millisecond delay per record to reduce disk/db overhead, so please allow appropriate time for ingestion (eg. ~10 seconds for 1000 records).
...
An `Import from smart-budget v3.2-` will be present for v4.X to allow pulling in data from past releases.
